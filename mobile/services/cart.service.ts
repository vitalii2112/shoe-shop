...
export const CartService = {
    get(): ICartState {
        return JSON.parse(storage.getString('cart') || '{"items": [], "isPayed": false}') as ICartState;
    },
    ...
    remove(id: number) {
        ...
        storage.set('cart', JSON.stringify({items: updatedCartItems, isPayed: false }));
    },
    ...
    setQuantity(id: number, value: number) {
        const cart = this.get()
        cart.items = cart.items.map(item => item.id === id ? {...item, quantity: value} : item)
        storage.set('cart', JSON.stringify(cart));
    }
};
